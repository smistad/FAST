...
macro(fast_add_python_interfaces GROUP)
    file(RELATIVE_PATH _relPath "${PROJECT_SOURCE_DIR}/source/" "${CMAKE_CURRENT_SOURCE_DIR}")
    foreach(_src ${ARGN})
        if(_relPath)
            list(APPEND FAST_PYTHON_HEADER_FILES_${GROUP} "${_relPath}/${_src}")
        else()
            list(APPEND FAST_PYTHON_HEADER_FILES_${GROUP} "${_src}")
        endif()
    endforeach()
    if (_relPath)
        # propagate to parent directory
        set(FAST_PYTHON_HEADER_FILES_${GROUP} ${FAST_PYTHON_HEADER_FILES_${GROUP}} PARENT_SCOPE)
    endif()
endmacro()

macro(fast_add_python_shared_pointers GROUP)
    foreach(_shared_ptr_object ${ARGN})
        list(APPEND FAST_PYTHON_SHARED_PTR_OBJECTS_${GROUP} ${_shared_ptr_object})
    endforeach()
        # propagate to parent directory
        set(FAST_PYTHON_SHARED_PTR_OBJECTS_${GROUP} ${FAST_PYTHON_SHARED_PTR_OBJECTS_${GROUP}} PARENT_SCOPE)
endmacro()
...
macro(fast_add_process_object GROUP NAME HEADERFILE)
    file (RELATIVE_PATH _relPath "${PROJECT_SOURCE_DIR}/source/" "${CMAKE_CURRENT_SOURCE_DIR}")
    list(APPEND FAST_PROCESS_OBJECT_NAMES_${GROUP} ${NAME})
    list(APPEND FAST_PYTHON_SHARED_PTR_OBJECTS_${GROUP} ${NAME})
    if(_relPath)
        list(APPEND FAST_PROCESS_OBJECT_HEADER_FILES ${_relPath}/${HEADERFILE})
        list(APPEND FAST_PYTHON_HEADER_FILES_${GROUP} ${_relPath}/${HEADERFILE})
    endif()
    if(_relPath)
        # propagate to parent directory
        set(FAST_PROCESS_OBJECT_NAMES ${FAST_PROCESS_OBJECT_NAMES} PARENT_SCOPE)
