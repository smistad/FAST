...
TEST_CASE("Us3Dhybrid point2plane intersection: Distance, Crossing, LocalCrossing", "[fast][Us3Dhybrid]") {    
    ...
    float distance = getPointDistanceAlongNormal(startPoint, planeBasePoint, planeNormal);
    CHECK(2.f == distance);
    Vector3f intersection = getIntersectionOfPlane(startPoint, distance, normalVector);
    CHECK(Vector3f(2.f, 2.f, 0.f) == intersection);
