...
SeededRegionGrowing::SeededRegionGrowing(float intensityMinimum, float intensityMaximum,
                                         std::vector<Vector3i> seedPoints, PixelConnectivity connectivity) : SeededRegionGrowing() {
    setIntensityRange(intensityMinimum, intensityMaximum);
    mSeedPoints = seedPoints;
    m_pixelConnectivity = connectivity;
}
    ...
    void SeededRegionGrowing::executeOnHost(T* input, Image::pointer output) {
        ...
        if(output->getDimensions() == 3) {
            for(int a = -1; a < 2; a++) {
                for(int b = -1; b < 2; b++) {
                    for(int c = -1; c < 2; c++) {
                        if(m_pixelConnectivity != PixelConnectivity::All) {
    ...
    void SeededRegionGrowing::execute() {
        ...
        if(getMainDevice()->isHost()) {
            ...
            auto access = output->getImageAccess(ACCESS_READ_WRITE);
            ...
            for(int i = 0; i < mSeedPoints.size(); i++) {
                ...
                access->setScalarFast3D<uchar>(pos, 2);
            }
            access->release();
        ...
        if(output->getDimensions() == 2) {
            mKernel.setArg(5, m_pixelConnectivity == PixelConnectivity::All ? 8 : 4);
        } else {
            mKernel.setArg(5, m_pixelConnectivity == PixelConnectivity::All ? 26 : 6);
        }
